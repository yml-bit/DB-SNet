...
from nnunetv2.training.data_augmentation.custom_transforms.deep_supervision_donwsampling import \
    DownsampleSegForDSTransform22,RenameTransform22
from nnunetv2.training.data_augmentation.custom_transforms.limited_length_multithreaded_augmenter import \
    LimitedLenWrapper
from nnunetv2.training.data_augmentation.custom_transforms.masking import MaskTransform
from nnunetv2.training.data_augmentation.custom_transforms.region_based_training import \
    ConvertSegmentationToRegionsTransform22
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            print(f"Total trainable parameters: {round(params * 1e-6, 3)} M")
            print(f"MAdds: {round(MACs * 1e-9, 3)} G")
    ...
    def _build_loss(self):
        ...
        if self.enable_deep_supervision:#修改
    ...
    def get_training_transforms(
        ...
        if deep_supervision_scales is not None:
            ...
            tr_transforms.append(RenameTransform22('seg', 'target1','target2', True))

        if regions is not None:
            # the ignore label must also be converted
            tr_transforms.append(ConvertSegmentationToRegionsTransform22(list(regions) + [ignore_label]
    ...
    def get_validation_transforms(
        ...
        if deep_supervision_scales is not None:
            ...
            val_transforms.append(RenameTransform22('seg', 'target1','target2', True))

        if regions is not None:
            # the ignore label must also be converted
            val_transforms.append(ConvertSegmentationToRegionsTransform22(list(regions) + [ignore_label]
                                                                        if ignore_label is not None else regions,
                                                                        'target', 'target1','target2'))
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.enable_deep_supervision:
            ...
            output1 = output[0]######需要修改
            output2 = output[0]
            target1 = target_task1
            target2 = target_task2
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        loss_here = 0.5 * loss_here1 + 0.5 * loss_her2
