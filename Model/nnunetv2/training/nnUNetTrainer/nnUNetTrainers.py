...
from nnunetv2.training.dataloading.data_loader_3d import nnUNetDataLoader3D,SequenceLoader3D
...
class nnUNetTrainers(nnUNetTrainer):
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            dataset = nnUNetDatasets(self.preprocessed_dataset_folder, case_identifiers=None,
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_tr = nnUNetDatasets(self.preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
        dataset_val = nnUNetDatasets(self.preprocessed_dataset_folder, val_keys,
                                    folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                    num_images_properties_loading_threshold=0)
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            dl_tr = SequenceLoader3D(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       sampling_probabilities=None, pad_sides=None)
            dl_val = SequenceLoader3D(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        sampling_probabilities=None, pad_sides=None)
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            train_outputs = []
            dl_tr_iter = iter(self.dataloader_train)
            for batch_id in range(self.num_iterations_per_epoch):
                # train_outputs.append(self.train_step(next(self.dataloader_train)))
                train_outputs.append(self.train_step(next(dl_tr_iter)))
            ...
            with torch.no_grad():
                ...
                val_outputs = []
                dl_val_iter = iter(self.dataloader_val)
                for batch_id in range(self.num_val_iterations_per_epoch):
                    val_outputs.append(self.validation_step(next(dl_val_iter)))
