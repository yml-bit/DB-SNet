...
class nnUNetTrainer(object):
    ...
    def on_train_epoch_start(self):
        self.network.train()
        self.scheduler_branch1.step(self.current_epoch)
        self.print_to_log_file('')
        self.print_to_log_file(f'Epoch {self.current_epoch}')
        self.print_to_log_file(
            f"Current learning rate: {np.round(self.optimizer_branch1.param_groups[0]['lr'], decimals=5)}")
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'optimizer_state': self.optimizer_branch1.state_dict(),
                    # 'optimizer_branch2_state': self.optimizer_branch2.state_dict(),
                    'grad_scaler_state': self.grad_scaler.state_dict() if self.grad_scaler is not None else None,
                    'logging': self.logger.get_checkpoint(),
                    '_best_ema': self._best_ema,  #self.emaa
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        # self.emaa =checkpoint['_best_ema']
