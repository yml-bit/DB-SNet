...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.initial_lr = 1e-4#1e-2
        self.weight_decay = 3e-5
        self.oversample_foreground_percent = 0.33
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50
        self.num_epochs = 100#400 600 800
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # self.optimizer_branch1, self.scheduler_branch1 = self.configure_optimizers()
    ...
    def configure_optimizers(self):
        ...
        for name, param in self.network.named_parameters():
            ...
            if 'stem' in name or 'enc_block_' in name or 'down_' in name or 'bottleneck' in name:
                ...
                param.requires_grad = False  # 冻结编码器参数
                    ...
                    'out_33' in name or 'dec_block_33' in name or 'up_33' in name or 'dummy_tensor' in name:
        ...
        self.print_to_log_file(f"Using optimizer {optimizer_branch2}")
        self.print_to_log_file(f"Using scheduler {scheduler_branch2}")
        return optimizer_branch2, scheduler_branch2
    ...
    def on_train_epoch_start(self):
        self.network.train()
        self.scheduler_branch2.step(self.current_epoch)
        self.print_to_log_file('')
        self.print_to_log_file(f'Epoch {self.current_epoch}')
        self.print_to_log_file(
            f"Current learning rate: {np.round(self.optimizer_branch2.param_groups[0]['lr'], decimals=5)}")
        # lrs are the same for all workers so we don't need to gather them in case of DDP training
        self.logger.log('lrs', self.initial_lr, self.current_epoch)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            l1 = self.loss(output[1], target_task1)
            # l = 0.6 * l1 + 0.4 * l2
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        global_dc_per_class,loss_here=self.validation_caculate(tp,fp,fn,loss1)
    ...
    def on_epoch_start(self):
        self.logger.log('epoch_start_timestamps', time(), self.current_epoch+1)
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'optimizer_state': self.optimizer_branch2.state_dict(),
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        # self.current_epoch =0# checkpoint['current_epoch']
        self.logger.load_checkpoint(checkpoint['logging'])
        for key in self.logger.my_fantastic_logging.keys():
            self.logger.my_fantastic_logging[key]=[self.logger.my_fantastic_logging[key][-1]]
        # self.logger.my_fantastic_logging['epoch_start_timestamps'] = self.logger.my_fantastic_logging['epoch_start_timestamps'][-1]
        # self.logger.my_fantastic_logging['epoch_end_timestamps']=self.logger.my_fantastic_logging['epoch_end_timestamps'][-1]
        self.logger.my_fantastic_logging['dice_per_class_or_region']=[self.logger.my_fantastic_logging['dice_per_class_or_region'][:4]]
        # self._best_ema = 0#checkpoint['_best_ema']
        ...
        self.optimizer_branch2, self.scheduler_branch2 = self.configure_optimizers()
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                if self.is_cascaded:###
                    data = np.vstack((data, convert_labelmap_to_one_hot(seg2[-1], self.label_manager.foreground_labels,
