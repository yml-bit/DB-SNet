...
from nnunetv2.inference.predict_from_raw_data_1 import nnUNetPredictor
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.optimizer_common=self.scheduler_common= None
        self.optimizer_branch1= self.scheduler_branch1=None
        self.optimizer_branch2 = self.scheduler_branch2 = None
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            ([self.optimizer_common, self.optimizer_branch1, self.optimizer_branch2],
             [self.scheduler_common, self.scheduler_branch1, self.scheduler_branch2])=self.configure_optimizers()
    ...
    def get_training_transforms(
        ...
        if deep_supervision_scales is not None:
            tr_transforms.append(DownsampleSegForDSTransform22(deep_supervision_scales, 0, input_key='target',
                                                              output_key1='target1',output_key2='target2'))
        tr_transforms.append(NumpyToTensor(['data', 'target1','target2'], 'float'))
    ...
    def get_validation_transforms(
        ...
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform22(deep_supervision_scales, 0, input_key='target',
                                                               output_key1='target1',output_key2='target2'))

        val_transforms.append(NumpyToTensor(['data', 'target1','target2'], 'float'))
    ...
    def on_train_epoch_start(self):
        self.network.train()
        self.scheduler_common.step(self.current_epoch)
        self.print_to_log_file('')
        self.print_to_log_file(f'Epoch {self.current_epoch}')
        self.print_to_log_file(
            f"Current learning rate: {np.round(self.optimizer_common.param_groups[0]['lr'], decimals=5)}")
        # lrs are the same for all workers so we don't need to gather them in case of DDP training
        self.logger.log('lrs', self.optimizer_common.param_groups[0]['lr'], self.current_epoch)
    ...
    def preprocess_label(self,label):
        ...
        for scale in [1, 1 / 2, 1 / 4, 1 / 8, 1 / 16]:
            ...
            data.append(Resize["label"].unsqueeze(1))
        return data
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if self.enable_deep_supervision:
            output1 = output[0][0]######需要修改
            output2 = output[1][0]
            target1 = target_task1[0]
            target2 = target_task2[0]
        ...
        if self.label_manager.has_ignore_label:
            if not self.label_manager.has_regions:
                mask1 = (target1 != self.label_manager.ignore_label).float()
                target1[target1 == self.label_manager.ignore_label] = 0
        ...
        tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot1, target1, axes=axes, mask=mask1)
        tp2, fp2, fn2, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot2, target2, axes=axes, mask=mask2)

        tp_hard = tp.detach().cpu().numpy()
        fp_hard = fp.detach().cpu().numpy()
        fn_hard = fn.detach().cpu().numpy()

        tp_hard2 = tp2.detach().cpu().numpy()
        fp_hard2 = fp2.detach().cpu().numpy()
        fn_hard2 = fn2.detach().cpu().numpy()
        if not self.label_manager.has_regions:
            tp_hard = tp_hard[1:]
            fp_hard = fp_hard[1:]
            fn_hard = fn_hard[1:]
        ...
        return {'loss1': l1.detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard,
                'loss2': l2.detach().cpu().numpy(), 'tp_hard2': tp_hard2, 'fp_hard2': fp_hard2, 'fn_hard2': fn_hard2}

    def validation_caculate(self,tp,fp,fn,loss):
        if self.is_ddp:
            world_size = dist.get_world_size()
            ...
            loss_here = np.mean(loss)

        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in zip(tp, fp, fn)]]
        return global_dc_per_class,loss_here
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        loss1=outputs_collated['loss1']

        tp2 = np.sum(outputs_collated['tp_hard2'], 0)
        fp2 = np.sum(outputs_collated['fp_hard2'], 0)
        fn2 = np.sum(outputs_collated['fn_hard2'], 0)
        loss2 = outputs_collated['loss2']
        global_dc_per_class1,loss_here1=self.validation_caculate(tp,fp,fn,loss1)
        global_dc_per_class2,loss_her2=self.validation_caculate(tp2,fp2,fn2,loss2)
        global_dc_per_class=global_dc_per_class1+global_dc_per_class2
        loss_here=0.6*loss_here1+0.4*loss_her2 ##不求平均？
        ...
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                ...
                checkpoint = {
                    ...
                    'optimizer_common_state': self.optimizer_common.state_dict(),
                    'optimizer_branch1_state': self.optimizer_branch1.state_dict(),
                    'optimizer_branch2_state': self.optimizer_branch2.state_dict(),
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        self.optimizer_common.load_state_dict(checkpoint['optimizer_common_state'])
        self.optimizer_branch1.load_state_dict(checkpoint['optimizer_branch1_state'])
        self.optimizer_branch2.load_state_dict(checkpoint['optimizer_branch2_state'])
        if self.grad_scaler is not None:
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            validation_output_folder1 = join(self.output_folder, 'validation1')
            maybe_mkdir_p(validation_output_folder1)
            validation_output_folder2 = join(self.output_folder, 'validation2')
            maybe_mkdir_p(validation_output_folder2)
            ...
            for k in dataset_val.keys():
                proceed = not check_workers_alive_and_busy(segmentation_export_pool, worker_list, results1,
                ...
                dd, seg2, properties = dataset_val.load_case(k)
                data=dd[0][np.newaxis,:, :, :]
                seg1=dd[1][np.newaxis,:, :, :]

                if self.is_cascaded:###
                    data = np.vstack((data, convert_labelmap_to_one_hot(seg1[-1], self.label_manager.foreground_labels,
                ...
                output_filename_truncated1 = join(validation_output_folder1, k)
                output_filename_truncated2 = join(validation_output_folder2, k)

                try:
                    prediction1,prediction2 = predictor.predict_sliding_window_return_logits(data)
                except RuntimeError:
                    ...
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_logits, (
                            (prediction1, properties, self.configuration_manager, self.plans_manager,
                             self.dataset_json, output_filename_truncated1, save_probabilities),
                        )
                    )
                )
                results2.append(
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_logits, (
                            (prediction2, properties, self.configuration_manager, self.plans_manager,
                             self.dataset_json, output_filename_truncated2, save_probabilities),
                        )
                    )
                )
                # for debug purposes
                    ...
                    for n in next_stages:
                        ...
                        try:
                            ...
                            tmp = nnUNetDataset(expected_preprocessed_folder, [k],
                                                num_images_properties_loading_threshold=0)
                            d, s, p = tmp.load_case(k)#data seg properties
                        ...
                        #                   self.dataset_json)
                        results1.append(segmentation_export_pool.starmap_async(
                            resample_and_save, (
                                (prediction1, target_shape, output_file, self.plans_manager,
                                 self.configuration_manager,
                                 properties,
                                 self.dataset_json),
                            )
                        ))
                        results2.append(segmentation_export_pool.starmap_async(
                            resample_and_save, (
                                (prediction2, target_shape, output_file, self.plans_manager,
                                 self.configuration_manager,
                                 properties,
                                 self.dataset_json),
                            )
            ...
            _ = [r.get() for r in results1]
            _ = [r.get() for r in results2]
        ...
        if self.local_rank == 0:
            ...
            #                                     validation_output_folder1,
            #                                     join(validation_output_folder1, 'summary.json'),
            ...
            metrics2 = compute_metrics_on_folder(join(self.preprocessed_dataset_folder_base, 'gt_segmentations'),
                                                validation_output_folder2,
                                                join(validation_output_folder2, 'summary.json'),
                                                self.plans_manager.image_reader_writer_class(),
                                                self.dataset_json["file_ending"],
                                                self.label_manager.foreground_regions if self.label_manager.has_regions else
                                                self.label_manager.foreground_labels,
                                                self.label_manager.ignore_label, chill=True)
            self.print_to_log_file("Validation complete", also_print_to_console=True)
            self.print_to_log_file("Mean Validation Dice-task2: ", (metrics2['foreground_mean']["Dice"]), also_print_to_console=True)
