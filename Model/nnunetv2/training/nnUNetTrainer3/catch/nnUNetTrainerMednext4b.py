from nnunetv2.nets.MTSNet.MTSMed import MedNeXt1a#原始
...
#128 160 160
class nnUNetTrainerMednext4b(nnUNetTrainer):
    def __init__(
            self,
            plans: dict,
            configuration: str,
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        model = MedNeXt1a(in_channels=num_input_channels, n_channels=32,  # 增大通道，减少卷积核l3
    ...
    def configure_optimizers(self):
        ...
        for name, param in self.network.named_parameters():
            ...
            if 'stem' in name or 'enc_block_' in name or 'down_' in name or 'bottleneck' in name or 'up_' in name:
                common_params.append(param)
                param.requires_grad = False  # 冻结编码器参数

            elif 'out_00' in name or 'dec_block_00' in name or \
                    'out_11' in name or 'dec_block_11' in name or \
                    'out_22' in name or 'dec_block_22' in name or \
                    'out_33' in name or 'dec_block_33' in name or 'dummy_tensor' in name:
                branch2_params.append(param)
                param.requires_grad = True
            elif 'out_0' in name or 'dec_block_0' in name or \
                    'out_1' in name or 'dec_block_1' in name or \
                    'out_2' in name or 'dec_block_2' in name or \
                    'out_3' in name or 'dec_block_3' in name:
