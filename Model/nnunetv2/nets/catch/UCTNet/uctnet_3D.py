...
def UnoB(tar,amap,S):
    ...
    for b in range(amap.shape[0]):
        for d in range(amap.shape[1]):
            for i in range(s,amap.shape[2]-s):
                for j in range(s,amap.shape[3]-s):
                    if tar[b,d,i,j]!=0:
                        if S == 3:
                            pro[b,d,i,j] = 0 if 4*tar[b,d,i,j] != (tar[b,d,i+1,j]+tar[b,d,i-1,j]+tar[b,d,i,j+1]+tar[b,d,i,j-1]) else 1
                        else:
                            pro[b,d,i,j] = 0 if S*S*tar[b,d,i,j] != np.sum(tar[b,d,i-s:i+s+1,j-s:j+s+1] * neibor) else 1
