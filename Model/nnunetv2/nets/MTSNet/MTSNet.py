...
class DynamicFusion22(nn.Module):
...
class EDecoderBlock11(nn.Module):
    def __init__(self, in_channels, n_filters, SE=False, ECA_size=5, reduction=16):
        ...
        self.deconv = nn.Sequential(nn.ConvTranspose3d(in_channels // 2, in_channels // 2, kernel_size=3,
                                          stride=2, padding=1, output_padding=1),
                                    nn.BatchNorm3d(in_channels // 2),
                                    nn.ReLU(inplace=True))

        self.conv3 = nn.Sequential(nn.Conv3d(in_channels // 2, n_filters, kernel_size=1),
                                   nn.BatchNorm3d(n_filters))
        self.expansion = 1
        ...
        if SE:#3D通道注意力机制
            self.se = nn.Sequential(
                nn.AdaptiveAvgPool3d(1),
                nn.Conv3d(n_filters * self.expansion, n_filters * self.expansion // reduction, kernel_size=1),
                nn.BatchNorm3d(n_filters * self.expansion // reduction),
                nn.ReLU(inplace=True),
                nn.Conv3d(n_filters * self.expansion // reduction, n_filters * self.expansion, kernel_size=1),
                nn.Sigmoid()
            )
        ...
        if ECA_size is not None: #3D高效通道注意力
            self.eca = nn.Sequential(
                nn.Conv3d(n_filters * self.expansion, n_filters * self.expansion, kernel_size=(1, ECA_size, ECA_size),
                          padding=(0, ECA_size // 2, ECA_size // 2), groups=n_filters * self.expansion),
                nn.BatchNorm3d(n_filters * self.expansion),
                nn.Sigmoid()
            )

        self.conv_out = nn.Conv3d(n_filters * self.expansion, in_channels// 2, kernel_size=(1, 1, 1), stride=(1, 1, 1), bias=False)
        self.conva=nn.Sequential(nn.Conv3d(in_channels, in_channels // 2, kernel_size=1),
                                 nn.GroupNorm(num_groups=in_channels // 2,num_channels=in_channels // 2),
                                 nn.ReLU(inplace=True))
        self.convb=nn.Sequential(nn.Conv3d(in_channels, in_channels // 2, kernel_size=1),
                                 nn.GroupNorm(num_groups=in_channels // 2,num_channels=in_channels // 2),
                                 nn.ReLU(inplace=True))

    def forward(self, xx):
        x1, x2, S1, S2=xx[0],xx[1],xx[2],xx[3]
        x1, _ = checkpoint.checkpoint(self.branch1_dec1, [x1, S1])
        x2, _ = checkpoint.checkpoint(self.branch2_dec1, [x2, S2])
        x = torch.cat((xx[0], xx[1]), dim=1)
        out = self.conv1(x)
        out = self.deconv(out)#上采样
        out = self.conv3(out)
        out = out + out * self.se(out)
        out = out + out * self.eca(out)
        y_out = self.conv_out(out)#??project
        x1= self.conva(torch.cat((x1, y_out), dim=1))
        x2=self.convb(torch.cat((x2, y_out), dim=1))
        return x1, x2
...
class UpDecoderBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.deconv_x = nn.ConvTranspose3d(in_channels, in_channels // 2, kernel_size=3,
                                           stride=2, padding=1, output_padding=1)
    def forward(self, x, h):
        x = self.deconv_x(x)#上采样
        h = self.deconv_h(h)#上采样
        # h = h + y_out

        return x, h

class DBSNet6r(nn.Module):
    def __init__(self,
                 in_channels: int,
                 n_channels: int,
                 n_classes: int,
        ...
        self.up_2 = UpDecoderBlock(8 * n_channels)
        # self.up_2 = Cross_attention_DecoderBlock(in_channels=8 * n_channels, n_filters=64)
        ...
        self.up_0 = UpDecoderBlock(2 * n_channels)
        # self.up_0=Cross_attention_DecoderBlock(in_channels=2 * n_channels, n_filters=64)
        ...
        self.up_22 = UpDecoderBlock(8 * n_channels)
        ...
        self.up_00 = UpDecoderBlock(2 * n_channels)
        self.dec_block_00 = nn.Sequential(*[
            MedNeXtBlock(
                in_channels=n_channels,
                out_channels=n_channels,
...
class DynamicFusion(nn.Module):
    ...
    def __init__(self, channels):
        ...
        # 跨模态注意力机制
        # self.cross_att = nn.MultiheadAttention(channels, num_heads=4)
        self.cross_att = CrossModalAttention3D(channels)

        # 自适应特征选择
        self.adaptive_fuse = nn.Sequential(
            nn.Conv3d(channels * 2, channels, 1),
            nn.GroupNorm(num_groups=channels,num_channels=channels),
            nn.LeakyReLU(0.2)
        )
        # 残差连接
        self.res_conv = nn.Conv3d(channels, channels, 1)

    def forward(self,x1, x2):
        # x1, x2=xx[0],xx[1]
        fused= self.cross_att(x1, x2)
        # 特征拼接融合
        cat_feat = torch.cat([x1, x2], dim=1)
        fused_cat = self.adaptive_fuse(cat_feat)

        # 残差连接
        output = fused + fused_cat + self.res_conv(x1)
        return output
...
class EnhancedDynStaFNet(nn.Module):
    ...
    def forward(self, xx):
        ...
        x1, x2 = checkpoint.checkpoint(self.cross_attention, [x1, x2])
        return x1, x2
...
class DMF_DecoderBlock(nn.Module):
    ...
    def forward(self, xx):
        ...
        attn = checkpoint.checkpoint(self.cross_attention, [x1, x2])
        x1 = x1 + attn  # 可学习权重调节
        x2 = x2 + attn
        return x1, x2
