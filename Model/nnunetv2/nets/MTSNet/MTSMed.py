...
class MedNeXt0a(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.stem(x)  # (2 32 56 160 256)
        x_res_0 = self.enc_block_0(x)  # enc_block_0 模块，已冻结
        x = self.down_0(x_res_0)  # down_0 模块，已冻结
        x_res_1 = self.enc_block_1(x)  # enc_block_1 模块，已冻结
        x = self.down_1(x_res_1)  # down_1 模块，已冻结
        x_res_2 = self.enc_block_2(x)  # enc_block_2 模块，已冻结
        x = self.down_2(x_res_2)  # down_2 模块，已冻结
        x_res_3 = self.enc_block_3(x)  # enc_block_3 模块，已冻结
        x = self.down_3(x_res_3)  # down_3 模块，已冻结
        x = self.bottleneck(x)  # bottleneck 模块，已冻结

        if self.outside_block_checkpointing:
            # x_res_0 = self.iterative_checkpoint(self.enc_block_0, x)  # 共同部分，已冻结
            # x = checkpoint.checkpoint(self.down_0, x_res_0, self.dummy_tensor)  # 共同部分，已冻结
            # x_res_1 = self.iterative_checkpoint(self.enc_block_1, x)  # 共同部分，已冻结
            # x = checkpoint.checkpoint(self.down_1, x_res_1, self.dummy_tensor)  # 共同部分，已冻结
            # x_res_2 = self.iterative_checkpoint(self.enc_block_2, x)  # 共同部分，已冻结
            # x = checkpoint.checkpoint(self.down_2, x_res_2, self.dummy_tensor)  # 共同部分，已冻结
            # x_res_3 = self.iterative_checkpoint(self.enc_block_3, x)  # 共同部分，已冻结
            # x = checkpoint.checkpoint(self.down_3, x_res_3, self.dummy_tensor)  # 共同部分，已冻结
            #
            # x = self.iterative_checkpoint(self.bottleneck, x)  # 共同部分，已冻结
            # if self.do_ds:
            #     x_ds_4 = checkpoint.checkpoint(self.out_4, x, self.dummy_tensor)
...
class MedNeXt2(nn.Module):
    def __init__(self,
        ...
        self.enhancer = SpatialAdaptiveEdgeEnhance3D(in_channels=n_channels)
        # self.cross_attention = CrossAttention3D(channel=n_channels, reduction=16)
        self.out_0 = OutBlock(in_channels=n_channels*2, n_classes=n_classes, dim=dim)
        self.out_00 = OutBlock(in_channels=n_channels*2, n_classes=n_classes, dim=dim)
        # self.cov_out_0 = nn.Conv3d(in_channels=n_channels, out_channels=n_classes,kernel_size=1)
        # Used to fix PyTorch checkpointing bug
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)
        if deep_supervision:
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            xx2=self.enhancer(x2)
            xx1 =self.enhancer(x1)
            # x1,x2= self.cross_attention(x1, x2)
            x1 =torch.concatenate([x1,xx2],dim=1)  # Enhanced x1 with cross-attention
            x2 = torch.concatenate([x2,xx1],dim=1) # Enhanced x1 with cross-attention
            x1 = checkpoint.checkpoint(self.out_0, x1, self.dummy_tensor)
            x2 = checkpoint.checkpoint(self.out_00, x2, self.dummy_tensor)
...
class MedNeXt2b(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            x1=x1+self.enhancer(x2)
            x2 = x2 + self.enhancer(x1)
            x1,x2= self.cross_attention(x1, x2)
            # net_out1 =torch.concatenate([att1+x1,att2],dim=1)  # Enhanced x1 with cross-attention
            # # Combine features
            # net_out2 = torch.concatenate([att2+x2,att1],dim=1) # Enhanced x1 with cross-attention
            x1 = checkpoint.checkpoint(self.out_0, x1, self.dummy_tensor)
            x2 = checkpoint.checkpoint(self.out_00, x2, self.dummy_tensor)
...
class MedNeXt3(nn.Module):
    ...
    def __init__(self,
        ...
        self.enc_block_00 = nn.Sequential(*[
            MedNeXtBlock(
                in_channels=n_channels,
                out_channels=n_channels,
                exp_r=exp_r[0],
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            x_res_0 = self.iterative_checkpoint(self.enc_block_0, x)
            x_res_00 = self.iterative_checkpoint(self.enc_block_00, x)
            x = checkpoint.checkpoint(self.down_0, x_res_0+x_res_00, self.dummy_tensor) #28
            x_res_1 = self.iterative_checkpoint(self.enc_block_1, x)
            x = checkpoint.checkpoint(self.down_1, x_res_1, self.dummy_tensor) #14
            x_res_2 = self.iterative_checkpoint(self.enc_block_2, x)
            x = checkpoint.checkpoint(self.down_2, x_res_2, self.dummy_tensor) #7
...
class CrossAttention3D(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        net_out1=spa_se_x1 + chn_se_x1+x1
        net_out2=spa_se_x2 + chn_se_x2+x2
