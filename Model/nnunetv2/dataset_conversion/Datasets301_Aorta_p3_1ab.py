...
from nnunetv2.dataset_conversion.generate_dataset_json import generate_dataset_json_ab
...
def copy_files(src_data_folder: Path, train_dir: Path, labelTr_dir: Path, test_dir: Path,labelTs_dir: Path):
    ...
    for path_str in patients_train:#内部数据用于开发，选择一个外部数据作为验证集。
        ...
        if file.name == "1.nii.gz":
            ...
            label_path2=path_str.replace("1.nii.gz","2.nii.gz")
            shutil.copy(label_path2, train_dir / f"Aorta_{str(num_cases).zfill(4)}_0001.nii.gz")

            label_path3=path_str.replace("1.nii.gz","3.nii.gz")
            shutil.copy(label_path3, labelTr_dir / f"Aorta_{str(num_cases).zfill(4)}.nii.gz")
...
def convert_Aorta(src_data_folder: str, dataset_id=301):
    ...
    generate_dataset_json_ab(
        str(out_dir),
        channel_names={
            0: "CTA",
            1: "task1",
        },
       ## 3.nii.gz-lesion;1正常，2-动脉瘤，3-夹层(2+1)，动脉瘤+夹层-4，5-狭窄(4+1)，（动脉瘤+狭窄，夹层+狭窄）归为一类6
        labels={    #defined task1,task2 not need define
            "background": 0,
            "blood": 1,
            "calcified": 2,
            "noncalcified": 3,
...
if __name__ == "__main__":
    ...
    input_folder="/media/bit301/data/yml/data/p4/internal/"  #相对路径下直接运行，命令行运行需要修改为绝对路径
    dataset_id=301
