...
import sys
base = '/media/bit301/data/yml/project/python310/p3/DATASET1ab'#512x512
# base = '/media/bit301/data/yml/project/python310/p3/DATASET1b'#300x256
# base=""
...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                # npy files
                proceed1 = not check_workers_alive_and_busy(export_pool, worker_list, r1, allowed_num_queued=2)
                while not proceed1:
                    # print('sleeping')
                    sleep(0.1)
                    proceed1 = not check_workers_alive_and_busy(export_pool, worker_list, r1, allowed_num_queued=2)

                proceed2 = not check_workers_alive_and_busy(export_pool, worker_list, r2, allowed_num_queued=2)
                while not proceed2:
                    # print('sleeping')
                    sleep(0.1)
                    proceed2 = not check_workers_alive_and_busy(export_pool, worker_list, r2, allowed_num_queued=2)

                results = self.predict_logits_from_preprocessed_data(data)
                prediction1 = results[0].cpu()
                prediction2 = results[1].cpu()
                ...
                if ofile is not None:
                    ...
                    r1.append(
                        export_pool.starmap_async(
                            convert_predicted_logits_to_segmentation_with_correct_shape, (
                                (prediction1, self.plans_manager,
                    ...
                    r2.append(
                        export_pool.starmap_async(
                            convert_predicted_logits_to_segmentation_with_correct_shape, (
                                (prediction2, self.plans_manager,
                                 self.configuration_manager, self.label_manager,
                                 properties,
                                 save_probabilities),)
                        )
            ...
            ret1 = [i.get()[0] for i in r1]
            ret2 = [i.get()[0] for i in r2]
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        results = self.predict_logits_from_preprocessed_data(dct['data'])
        predicted_logits1=results[0].cpu()
        predicted_logits2=results[1].cpu()

        if self.verbose:
            print('resampling to original shape')
        if output_file_truncated is not None:
            export_prediction_from_logits(predicted_logits1, dct['data_properties'], self.configuration_manager,
                                          self.plans_manager, self.dataset_json, output_file_truncated,
                                          save_or_return_probabilities)
            export_prediction_from_logits(predicted_logits2, dct['data_properties'], self.configuration_manager,
                                          self.plans_manager, self.dataset_json, output_file_truncated,
                                          save_or_return_probabilities)
        else:
            ret1 = convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits1, self.plans_manager,
                                                                              self.configuration_manager,
                                                                              self.label_manager,
                                                                              dct['data_properties'],
                                                                              return_probabilities=
                                                                              save_or_return_probabilities)
            ret2 = convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits2, self.plans_manager,
                                                                               self.configuration_manager,
                                                                               self.label_manager,
                                                                               dct['data_properties'],
                                                                               return_probabilities=
        ...
        with torch.no_grad():
            ...
            for params in self.list_of_parameters:
                ...
                if prediction1 is None:
                    results = self.predict_sliding_window_return_logits(data)
                    prediction1 = results[0].to('cpu')
                    prediction2 = results[1].to('cpu')
                else:
            ...
            if len(self.list_of_parameters) > 1:
                prediction1 /= len(self.list_of_parameters)
                prediction2 /= len(self.list_of_parameters)

            if self.verbose: print('Prediction done')
            prediction1 = prediction1.to('cpu')
            prediction2 = prediction2.to('cpu')
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        prediction1,prediction2 = self.network(x)
        if isinstance(prediction1,list):#新增
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                ...
                catch1,catch2=self.network(torch.flip(x, (*axes,)))
                if isinstance(catch1, list):#新增加
                    catch1 = catch1[0]
                    catch2 = catch2[0]
                prediction1 += torch.flip(catch1, (*axes,))
                prediction2 += torch.flip(catch2, (*axes,))
            prediction1 /= (len(axes_combinations) + 1)
            prediction2 /= (len(axes_combinations) + 1)
        return prediction1,prediction2
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        predicted_logits1 = torch.zeros((self.label_manager.num_segmentation_heads, *data.shape[1:]),
                                       dtype=torch.half,
                                       device=results_device)
        predicted_logits2 = torch.zeros((self.label_manager.num_segmentation_heads, *data.shape[1:]),
                                       dtype=torch.half,
                                       device=results_device)
        n_predictions1 = torch.zeros(data.shape[1:], dtype=torch.half, device=results_device)
        n_predictions2 = torch.zeros(data.shape[1:], dtype=torch.half, device=results_device)
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            results = self._internal_maybe_mirror_and_predict(workon)
            prediction1=results[0][0].to(results_device)
            prediction2=results[1][0].to(results_device)

            predicted_logits1[sl] += (prediction1 * gaussian if self.use_gaussian else prediction1)
            n_predictions1[sl[1:]] += (gaussian if self.use_gaussian else 1)
            predicted_logits2[sl] += (prediction2 * gaussian if self.use_gaussian else prediction2)
            n_predictions2[sl[1:]] += (gaussian if self.use_gaussian else 1)

        predicted_logits1 /= n_predictions1
        predicted_logits2 /= n_predictions2
        # check for infs
        if torch.any(torch.isinf(predicted_logits1)):
            raise RuntimeError('Encountered inf in predicted array. Aborting... If this problem persists, '
                               'reduce value_scaling_factor in compute_gaussian or increase the dtype of '
                               'predicted_logits to fp32')
        return predicted_logits1,predicted_logits2
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        predicted_logits1,predicted_logits2 = self._internal_predict_sliding_window_return_logits(data, slicers, self.perform_everything_on_device)
                    except RuntimeError:
                        print('Prediction on device was unsuccessful, probably due to a lack of memory. Moving results arrays to CPU')
                        empty_cache(self.device)
                        predicted_logits1,predicted_logits2 = self._internal_predict_sliding_window_return_logits(data, slicers, False)
                else:
                    predicted_logits1,predicted_logits2 = self._internal_predict_sliding_window_return_logits(data, slicers, self.perform_everything_on_device)

                empty_cache(self.device)
                # revert padding
                predicted_logits1 = predicted_logits1[tuple([slice(None), *slicer_revert_padding[1:]])]
                predicted_logits2 = predicted_logits2[tuple([slice(None), *slicer_revert_padding[1:]])]
        return predicted_logits1,predicted_logits2
...
class Inference():
    def __init__(self,id:int,
        ...
        self.predictor = nnUNetPredictor(
            model=model,
            tile_step_size=0.75,  # p3_1a:0.5, p3_1b:0.75
            use_gaussian=True,
            use_mirroring=True,
            perform_everything_on_device=True,
        ...
        self.model=model
        model_path1 = 'Dataset301_Aorta/' + self.model + '__nnUNetPlans__3d_fullres'
        model_path2 = 'Dataset301_Aorta/' + self.model + '__nnUNetPlans__2d'
        m1=os.path.join(nnUNet_results,model_path1)
        m2=os.path.join(nnUNet_results,model_path2)
    ...
    def predict(self, input_dir, output_dir):
        path_list = []
        inputs = "/media/bit301/data/yml/data/p4/external"  # external time
        for root, dirs, files in os.walk(inputs, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                if "1.nii.gz" in path:#and "cq" not in path
                # if "1.nii.gz" in path and "lz" in path:
                    path_list.append(path)

        # f1 = open("/media/bit301/data/yml/project/python310/p3/LightMUNet/nnunetv2/inference/disp.txt")  # 564
        # path_list = []
        # for line in f1.readlines():  # tile_step_size=0.75较好处理官腔错位问题
        #     path = line.split('\n')[0]
        #     if "1.nii.gz" in path:
        #         path_list.append(path)
        ...
        for path in path_list:
            # if self.model=="nnUNetTrainerMTLSegFormer":
            #     if ij < 125:
            #         ij = ij + 1
            #         continue
            if self.model=="nnUNetTrainerDSNet6r":
                if ij < 250:
                    ij = ij + 1
                    continue
            ...
            label = sitk.GetImageFromArray(ret[0].astype(np.int16))
            out="p4test/a/"+self.model#
            out_put = path.replace("p4", out).replace("1.nii.gz","2.nii.gz")
            target_directory = os.path.dirname(out_put)
            os.makedirs(target_directory, exist_ok=True)
            sitk.WriteImage(label, out_put)

            label = sitk.GetImageFromArray(ret[1].astype(np.int16))
            out = "p4test/b/" + self.model  #
            out_put = path.replace("p4", out).replace("1.nii.gz", "3.nii.gz")
            target_directory = os.path.dirname(out_put)
            os.makedirs(target_directory, exist_ok=True)
            sitk.WriteImage(label, out_put)
            ij += 1
...
if __name__ == '__main__':
    ...
    # id=0
    # model='nnUNetTrainerMednext1'# nnUNetTrainerMednext nnUNetTrainerMaCNN2
    inference = Inference(id,model)
    input_dir = "a"
    output_dir="b"
    inference.predict(input_dir, output_dir)#