...
import sys
base = '/media/bit301/data/yml/project/python310/p3/DATASET1a'#512x512
...
class Inference():
    ...
    def predict_1b(self, input_dir, output_dir):
        path_list = []
        # inputs = input_dir  # 输入目录
        inputs = "/media/bit301/data/yml/data/p4/external/"  # pre
        # inputs = "/media/bit301/data/yml/data/p3/external/cq/dis/dml/PA5"
        # inputs="/media/bit301/data/yml/data/p3/external/cq/dis/dml/PA19"###升主动脉
        for root, dirs, files in os.walk(inputs, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                if "1.nii.gz" in path:
                    path_list.append(path)
                # if "0.nii.gz" in path and "hnnk" in path:
                #     path_list.append(path)

        ij = 0
        for path in path_list:
            # if "nnUNetTrainerMednext"==self.model:
            #     if ij < 371:
            #         ij = ij + 1
            #         continue
            img, props = SimpleITKIO().read_images([path])
...
if __name__ == '__main__':
    ...
    # id=0
    # model='nnUNetTrainerMaCNN3'# nnUNetTrainerMednext nnUNetTrainerMaCNN2
    inference = Inference(id,model)
    input_dir = "a"
    output_dir="b"
    inference.predict_1a(input_dir, output_dir)#
    # inference.predict_1b(input_dir, output_dir)#